...
_RUNNER = (
    ...
    "workspace=_canonical_path(sys.argv[3])\n"
    "result_path=sys.argv[5]\n"
    "problem_root=_canonical_path(sys.argv[6])\n"
    "allowed_problem_files={_canonical_path(item) for item in json.loads(sys.argv[7])}\n"
    ...
    "    except Exception:\n"
    "        pass\n"
    "entrypoint=_canonical_path(sys.argv[1])\n"
    ...
    "    try:\n"
    "        return _canonical_path(os.fsdecode(os.fspath(value)))\n"
...
_EMBEDDED_EVALUATE_RUNNER = (
    ...
    "    absolute=os.path.abspath(path)\n"
    "    return os.path.normcase(os.path.realpath(absolute))\n"
    "workspace=_canonical_path(sys.argv[1])\n"
    "primary_path=_canonical_path(sys.argv[2])\n"
    "stage=json.loads(sys.argv[3])\n"
    "result_path=sys.argv[4]\n"
    "allowed_workspace_files={_canonical_path(item) for item in json.loads(sys.argv[5])}\n"
...
_PREFLIGHT_RUNNER = (
    ...
    "    absolute=os.path.abspath(path)\n"
    "    return os.path.normcase(os.path.realpath(absolute))\n"
    "problem_root=_canonical_path(sys.argv[2])\n"
    "allowed_problem_files={_canonical_path(item) for item in json.loads(sys.argv[3])}\n"
    "def emit(ok,message=''):\n"
    "    print(json.dumps({'ok':ok,'message':message}))\n"
    "    sys.exit(0 if ok else 1)\n"
    "entrypoint=_canonical_path(path)\n"
    "validate_dir=os.path.dirname(entrypoint)\n"
    "allowed_problem_files.add(entrypoint)\n"
    "is_package=os.path.basename(entrypoint)=='__init__.py'\n"
